...
import { fetchNotes, fetchNotesByRange, upsertNote } from '../api/notes'
...
export default function ComparePage({ cards, cloud, cloudAccount }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchServiceCosts(yearA, monthA, { min_cost: 0, cloud, cloud_account: cloudAccount }),
      fetchServiceCosts(yearB, monthB, { min_cost: 0, cloud, cloud_account: cloudAccount }),
    ])
      .then(([resA, resB]) => {
  ...
  useEffect(() => {
    ...
    const [yearB, monthB] = selB.split('-').map(Number)
    fetchNotes(yearB, monthB, cloud, cloudAccount)
      .then((items) => {
  ...
  useEffect(() => {
    ...
    fetchResources(formService, cloud, cloudAccount)
      .then((res) => {
  ...
  }, [formService, cloud, cloudAccount])
  ...
  const handleSave = async () => {
    ...
    try {
      const saved = await upsertNote({
        ...
        cloud,
        cloud_account: cloudAccount || 'default',
      })
  ...
  const loadSummary = useCallback((from: string, to: string) => {
    ...
    fetchNotesByRange(from, to, cloud, cloudAccount)
      .then(setSummaryNotes)
      .catch(() => setSummaryNotes([]))
      .finally(() => setSummaryLoading(false))
  }, [cloud, cloudAccount])

  useEffect(() => { loadSummary(summaryFrom, summaryTo) }, [cloud, cloudAccount])
